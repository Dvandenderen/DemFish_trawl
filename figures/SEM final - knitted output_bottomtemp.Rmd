...
## SEM model bottom temp sensitivity
...
Results are obtained with Rpackage Lavaan. Since demersal fish are associated with bottom waters, sea surface temperature (SST) may not match with the ambient temperature that these fish experience. This difference is likely to be low as all surveyed areas are relatively shallow. We choose SST as this allowed us to use the same dataset for all our analysis, whereas temperature information for the near-bottom layer does not cover the entire time series.
We verified some of our results using near-bottom temperature information. 
...
load (file = "C:/Users/danie/Documents/Online for git/DemFish_trawl/figures/my_work_space_watercolT.RData")

# re-scale parameters
semparam <- c("NPP","z_prod","biomass","tlw","T_time","ER_log")

## hypothesized model lavaan
modT2='biomass ~ tlw + ER_log + T_time + z_prod
       tlw ~ NPP + ER_log
       z_prod ~ NPP +  T_time  
       NPP ~ T_time
       T_time ~~ 0*ER_log'
...
# piecewise SEM
model <- psem(
  lm(biomass ~ tlw + ER_log + T_time + z_prod, data=datbio),
  lm(tlw ~ NPP + ER_log, data=datbio),
  lm(z_prod ~ NPP +  T_time , data=datbio),
  lm(NPP ~ T_time , data=datbio),
  datbio)
...
write.csv(SEM_out,file="C:/Users/danie/Documents/Online for git/DemFish_trawl/figures/SEM_out_waterT.csv")
