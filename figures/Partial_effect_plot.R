...
pdf("figures/Partial effect_supplement.pdf",width=7.5,height=2.8)
par(mfrow=c(1,4), mar=c(6, 4, 2, 1))

mod1 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat1)
mod2 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat2)
mod3 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat3)
mod4 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat4)
mod5 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat5)
mod6 <- lm(biomass~ LER + SST_time + tlw + ben_prod + lz_prod, data=dat6)
...
effect(mod1,"lz_prod",0.95,x.label="Zoop. prod.",rug =F,y.label = "",  line.color='blue',ci.color = "white")
effect2(mod2,"lz_prod",0.95,x.label="",rug =F,y.label = "", line.color='blue',line.type = 3)
effect2(mod3,"lz_prod",0.95,x.label="",rug =F,y.label = "", line.color='blue',line.type = 5)
effect2(mod4,"lz_prod",0.95,x.label="",rug =F,y.label = "", line.color='red')
effect2(mod5,"lz_prod",0.95,x.label="",rug =F,y.label = "", line.color='red',line.type = 3)
effect2(mod6,"lz_prod",0.95,x.label="",rug =F,y.label = "", line.color='red',line.type = 5)
