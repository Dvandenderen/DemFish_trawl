...
# re-scale parameters
semparam <- c("NPP","z_prod","biomass","tlw","SST_time","ER_log","depth","ben_prod")

## hypothesized model lavaan
modT2='biomass ~ tlw + ER_log + SST_time + z_prod + ben_prod
       tlw ~ NPP + ER_log
       z_prod ~ NPP +  SST_time  
       ben_prod ~ NPP + SST_time + depth + z_prod
       NPP ~ SST_time
       SST_time ~~ 0*ER_log'
...
Results show that the benthic flux to biomass is 6/6 times non-significant. Re-doing the analysis for the entire time series (see below) supports these findings (note that trophic level is now not very important). Final model is simplified and  depth and detrital bottom flux are excluded.  
