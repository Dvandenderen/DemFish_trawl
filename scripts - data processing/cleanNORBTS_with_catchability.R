...
norw_dat <- cbind(norw_dat,keep[match(norw_dat$AphiaID,keep$AphiaID),c("Genus","Family")])

norw_dat <- norw_dat %>%
  mutate(StatRec=NA) %>% 
  select(Survey, HaulID, StatRec, Year, Month, Quarter, Season, ShootLat, ShootLong, HaulDur, Area.swept,Area.doors, Gear, Depth, SBT, SST, Family, 
         Genus, Species, AphiaID, numcpue, wtcpue, numh, wgth, Length, numlencpue, numlenh)

rm(df_test, keep_sp, keep, keep_ap,keep_fa,keep_gen, my_sp_taxo, norw_dat0, sp_list, sp_list_change, sp_list_ok, sp.pb, check.ab, check.sp, check.sub.ab)
...
lengthcl <- norw_dat[,c(2,19,26,27,35,36,37,38,42:45)]
lengthcl <- lengthcl %>%
  distinct() %>%
