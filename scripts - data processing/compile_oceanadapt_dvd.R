# ------------------------------------------------------------------------------
# script adapted from oceanadapt processing script 
# downloaded July 2021
# ------------------------------------------------------------------------------

require(devtools)
install_version("readr", repos = "https://mran.revolutionanalytics.com/snapshot/2019-12-05/")

# Load required packages 
 library(readr)       # use above 
 library(tidyverse)   # version 1.3.1
 library(lubridate)   # version 1.7.10
 library(PBSmapping)  # version 2.73.0
 #library(gridExtra) 
 #library(questionr) 
 #library(geosphere)
...
wctri <- subset(wctri,wctri$DISTANCE_FISHED > .1)
...
wctri <- wctri %>% 
  ...
  mutate(
    ...
    wtcpue = (WEIGHT*10000)/(DISTANCE_FISHED*1000*NET_WIDTH) # KG per tow --> KG per area swept in m2 --> KG per hectare
  )
...
wcann <- subset(wcann,wcann$area_swept_ha_der > 0.15) # remove distance below approx. 100 m with gear of 15 m
wcann <- wcann %>% 
  mutate(
    # create haulid
    haulid = trawl_id,
...
gmex <- gmex %>% 
  filter(MIN_FISH <= 60 & MIN_FISH > 5 & !is.na(MIN_FISH)) %>% 
...
big_fall <- data.table::data.table(svspp = c('013', '022', '024', '027', '028', 
                                            '031', '033', '034', '073', '076', 
                                            '106', '107', '109', '121', '135', 
                                            '136', '141', '143', '145', '149', 
                                            '155', '164', '171', '181', '193', 
...
big_spring <- data.table::data.table(svspp = c('013', '022', '024', '027', '028', 
                                            '031', '033', '034', '073', '076', 
                                            '106', '107', '109', '121', '135', 
                                            '136', '141', '143', '145', '149', 
                                            '155', '164', '171', '181', '193', 
...
neus_fall_haul <- read_csv(here::here("data_raw", "neus_fall_svsta.csv"), col_types = cols(.default = col_character())) %>% 
  select("CRUISE6","STRATUM", "ID", "AREA","EST_YEAR","AVGDEPTH", "DECDEG_BEGLAT","DECDEG_BEGLON","DECDEG_ENDLAT","DECDEG_ENDLON", "SVVESSEL")
...
start <- as.data.frame(neus_fall[,c("DECDEG_BEGLON", "DECDEG_BEGLAT")], nrow = nrow(neus_fall), ncol = 2)
start[,1] <- as.numeric(start[,1]) ; start[,2] <- as.numeric(start[,2])
# create a matrix of ending positions
end <- as.data.frame(neus_fall[,c("DECDEG_ENDLON", "DECDEG_ENDLAT")], nrow = nrow(neus_fall), ncol = 2)
end[,1] <- as.numeric(end[,1]) ; end[,2] <- as.numeric(end[,2])
# add distance to neus_fall table
neus_fall$distance <- geosphere::distHaversine(p1 = start, p2 = end) # no data before 1996
neus_fall$distance[neus_fall$distance > 4*10^5] <- NA # remove one very large distance
neus_fall$distance[neus_fall$distance < 100] <- NA     # and a few very short
# for 1968-2005, use 30 minutes x 3.5 knots = 3241 meter distance - see NAFO SCR Doc. 14/024 Serial No. N6319
neus_fall$distance <- ifelse(is.na(neus_fall$distance),3241,neus_fall$distance) 
neus_fall <- subset(neus_fall,neus_fall$EST_YEAR > 1967) # remove all data before 1968
neus_fall$wtcpue <- as.numeric(neus_fall$EXPCATCHWT) / (13 * neus_fall$distance/10000) # kg per tow changed to kg per hectare
...
neus_fall$SVSPP <- str_pad(neus_fall$SVSPP, 3, pad = "0")
...
# DvD: some species are not corrected based on the information
neus_fall$wtcpue <- ifelse((neus_fall$SVVESSEL %in% c('HB', 'PC') & !(neus_fall$SVSPP %in% big_fall$svspp)), neus_fall$wtcpue / mean(big_fall$rhoW),neus_fall$wtcpue)
...
rm(neus_catch_clean, neus_catch_raw, neus_fall_catch, neus_fall_haul,start,end)
...
# calculate trawl distance in order to calculate effort
# create a matrix of starting positions
start <- as.data.frame(neus_spring[,c("DECDEG_BEGLON", "DECDEG_BEGLAT")], nrow = nrow(neus_spring), ncol = 2)
start[,1] <- as.numeric(start[,1]) ; start[,2] <- as.numeric(start[,2])
# create a matrix of ending positions
end <- as.data.frame(neus_spring[,c("DECDEG_ENDLON", "DECDEG_ENDLAT")], nrow = nrow(neus_spring), ncol = 2)
end[,1] <- as.numeric(end[,1]) ; end[,2] <- as.numeric(end[,2])
# add distance to neus_spring table
neus_spring$distance <- geosphere::distHaversine(p1 = start, p2 = end) # data in 1971 and after 1995
neus_spring$distance[neus_spring$distance < 100] <- NA     # and a few very short
# for 1968-2005, use 30 minutes x 3.5 knots = 3241 meter distance - see NAFO SCR Doc. 14/024 Serial No. N6319
neus_spring$distance <- ifelse(is.na(neus_spring$distance),3241,neus_spring$distance) 
neus_spring <- subset(neus_spring,neus_spring$EST_YEAR > 1967) # remove all data before 1968
neus_spring$wtcpue <- as.numeric(neus_spring$EXPCATCHWT) / (13 * neus_spring$distance/10000) # kg per tow changed to kg per hectare
...
neus_spring$SVSPP <- str_pad(neus_spring$SVSPP, 3, pad = "0")
...
for(i in 1:length(dcf.spp)){
   neus_spring[year < 1985 & SVSPP == dcf.spp[i], wtcpue := wtcpue * NEFSC_conv[SVSPP == dcf.spp[i], DCF_WT]]
}
...
# DvD: some species are not corrected based on the information
neus_spring$wtcpue <- ifelse((neus_spring$SVVESSEL %in% c('HB', 'PC') & !(neus_spring$SVSPP %in% big_spring$svspp)), neus_spring$wtcpue / mean(big_spring$rhoW),neus_spring$wtcpue)
...
rm(neus_spp, neus_strata, big_fall, big_spring, NEFSC_conv,start,end)
...
seus <- seus %>%
  ...
  mutate(EFFORT = (15 * distance_m)/10000, 
         # Create a unique haulid
         haulid = EVENTNAME, 
         # Extract year where needed
...
seus$distance_m[seus$distance_m >4e+4] <- NA   
seus$distance_m[seus$distance_m < 100]  <- NA
seus$EFFORT[seus$EFFORT > 80]  <- NA  
...
mar_catch <- mar_catch %>%
  distinct(MISSION,SETNO,SPEC,TOTWGT,TOTNO) 
...
# get wingspread from ices datras per gear type
mar$wingspread <- NA
mar$wingspread[mar$geardesc == "Western IIA trawl"] <- 12.5
mar$wingspread[mar$geardesc == "Yankee #36 otter trawl"] <- 10.7

# estimate kg per hectare from kg per tow 
mar$wtcpue <- mar$totwgt / (mar$wingspread*mar$dist*1.852*1000/10000)
...
rm(mar_catch, mar_info, mar_missions, mar_spp, mission_files, info_files, spp_files, catch_files,test)
...
rm(marSpring, marSummer, marFall)
...
QCS <- left_join(QCS_catch, QCS_effort, by = c("Trip.identifier", "Set.number","Survey.Year"))
  ...
  QCS <- QCS %>% 
    ...
    mutate(
      ...
      # catch weight (kg.) per tow to kg per hectare (assume relationship wingspread = 0.3 x doorspread)	
      wtcpue = (Catch.weight..kg.)/(Distance.towed..m.*Trawl.door.spread..m.*0.3/10000)
    )
...
rm(SOG, SOG_catch, SOG_effort, SOG_strats, QCS, QCS_catch, QCS_effort, QCS_strats, 
  ...
  WCV, WCV_catch, WCV_effort, WCV_strats,files,test,test2)
...
# gear wing spread - see Hurlbut and Clay (1990) Protocols for Research Vessel Cruises
GSLsouth$wingspread <- NA
GSLsouth$wingspread[GSLsouth$gear.str == "Western IIA trawl"] <- 12.5
GSLsouth$wingspread[GSLsouth$gear.str == "Yankee #36 otter trawl"] <- 10.7
...
GSLsouth <- GSLsouth %>% 
  ...
  mutate(
    ...
    # OceanAdapt: distance, 1.75 nau mi. (via Daniel Ricard) = 3241 m. 
    # additional note: estimate is based on Hurlbut and Clay (1990) Protocols for Research Vessel Cruises (Table 1.1)
    # catch weight (kg.) per tow to kg per hectare	
    wtcpue = (weight.caught)/(3241 * wingspread/10000)
  )
...
GSLnor <- subset(GSLnor,GSLnor$Dist_Towed > 0.1 & GSLnor$Dist_Towed < 20)
...
GSLnor <- GSLnor %>%
  ...
  mutate(
    ...
    #weight of catch (kg.) per tow to kg per hectare
    wtcpue = (Pds_Capture)/(Dist_Towed*1.852*1000 *wingspread/10000)
  )
...
tax <- tax[!duplicated(tax[,c('taxon')]),]
...
if(isTRUE(WRITE_MASTER_DAT)){
  if(isTRUE(PREFER_RDATA)){
    saveRDS(dat, file = here::here("data_clean", "all-regions-full-dvd-220222.rds"))
  }else{
