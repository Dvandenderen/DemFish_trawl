...
dat$LER <- log10(dat$ER)
mod1 <- lm(biomass~ LER + SST_time + tlw + z_prod, data=dat)
dat$ix <- predict(mod1,newdata=dat)

dat$ix[6] <- 0
dat2 <- dat# dat[-6,]
mod1 <- lm(log10(dat2$ix/1000+1)~log10(dat2$biomass/1000))
x <- c(-0.25,3.5)
y <- mod1$coefficients[1] + mod1$coefficients[2]*x
plot(log10(dat2$ix/1000+1),log10(dat2$biomass/1000),xlab="Predicted biomass"
     ,ylab="Observed biomass",xaxt="n",yaxt="n",xlim=c(-0.25,2.7),ylim=c(-0.25,2.7),pch=16)
axis(1,c(0,1,2),c("1","10","100"))
axis(2,c(0,1,2),c("1","10","100"),las=1)
abline(0,1)
