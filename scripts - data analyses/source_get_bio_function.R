...
get_bio<-function(filename,t_start, t_end, spatialunit){
  ...
  ecodat <- grid@data
  ecodat[,c(7:18,22:26)]  <- ecodat[,c(7:18,22:26)] * ecodat$ocean_sqkm # weighted with the size of the ocean area
  
  tt <-  aggregate(list(ecodat[,c(4,7:18,22:27)]),
                   by=list(ecodat[,spatialunit]),FUN=sum,na.rm=T)
  cnam <- colnames(ecodat[,c(4,7:18,22:27)])
