...
grid_master@data$ECO_REG <- ifelse(grid_master@data$uni_cell == "ID_794","Celtic Seas",grid_master@data$ECO_REG)
grid_master@data$ECO_REG <- ifelse(grid_master@data$uni_cell == "ID_292","Puget Trough/Georgia Basin",grid_master@data$ECO_REG)
...
mfs <- leaflet() %>%
  ...
  # Layers control
  addLayersControl(
    overlayGroups = c("grid","subdiv","ecoreg"),
