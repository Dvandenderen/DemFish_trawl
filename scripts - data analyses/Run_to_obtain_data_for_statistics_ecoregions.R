...
cpue_final <- cbind(cpue_final,depth_grid[match(cpue_final$one_degrees,depth_grid$one_degrees),c("depth")])
colnames(cpue_final)[ncol(cpue_final)] <- "depth"
cpue_final <- subset(cpue_final,cpue_final$depth < 200)
cpue_final <- subset(cpue_final,cpue_final$depth > 50)
...
tnew <- cbind(cpue_final,depth_grid[match(cpue_final$one_degrees,depth_grid$one_degrees),c("depth")])
colnames(tnew)[ncol(tnew)] <- "depth"

ttdepth <-  aggregate(list(tnew$depth),  by=list(tnew$ECO_REG,tnew$year),FUN=mean)
colnames(ttdepth) <- c("ECO_REG","year","depth")

## plot timeseries of catch, biomass
 par(mfrow = c(3, 4))
 ...
 # get depth observaiton
 timeser <- cbind(timeser,ttdepth[match(timeser$EcReg,ttdepth$ECO_REG),c(3)])
 colnames(timeser)[ncol(timeser)] <- "Depth_obs"
 
 setwd("C:/Users/danie/Desktop")
 save(timeser,file = "datasub_ecoRegions.Rdata")
