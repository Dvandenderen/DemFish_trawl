...
# --------------------------------------------------------------------------------
# select all grid cells that are regularly sampled 
# --------------------------------------------------------------------------------
  ...
  cpue_good <- cbind(cpue_good,grid_master@data[match(cpue_good$uni_cell,grid_master@data$uni_cell),c("ocean_sqkm")])
...
# --------------------------------------------------------------------------------
  ...
  for(j in 1:nrow(cpue_good)){
    coll <- which(cpue_good$year[j] == colnames(sstdat))
    row  <- which(cpue_good$uni_cell[j] == sstdat$uni_cell)
    if (length(coll)>0){
...
# --------------------------------------------------------------------------------
  ...
  cpue_good$SBT <- NA
  for(j in 1:nrow(cpue_good)){
    coll <- which(cpue_good$year[j] == colnames(tdat))
    row  <- which(cpue_good$uni_cell[j] == tdat$uni_cell)
    if (length(coll)>0){
      cpue_good$SBT[j] <- tdat[row,coll]
    }}
