...
source("scripts - data processing/source_combine_all_surveys_after_cleaning.R") # check why INF numbers? 
...
trawl_sep <- trawl %>% 
  group_by(haulid,region,gear,year,month,lon,lat,type,size) %>%
  summarize_at(.vars=c('wtcpue', 'wtcpue_q'), .funs = function(x) sum(x)) %>% 
  dplyr::select(haulid,region,gear,year,month,lon,lat,type,size,wtcpue,wtcpue_q) %>%
  as.data.frame()
...
trawl <- trawl %>% 
  ...
  # add dem 
trawl_sep <- subset(trawl_sep,trawl_sep$type =="dem" & trawl_sep$size =="large")
